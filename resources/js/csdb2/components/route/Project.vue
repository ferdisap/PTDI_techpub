  ...
  <div class="w-full flex">
      ...
      <table>
        ...
        <tbody>
          <tr v-for="pr in techpubStore.Project" class="hover:bg-blue-400">
            <td> <router-link :to="{name:'ProjectDetail', params:{projectName: pr.name},}">{{ pr.name }}</router-link> </td>
    ...
    <div class="block ml-3">
      ...
      <div v-if="showCreateRepo" class="mt-3 w-full max-w-2/3">
        <form :action="this.techpubStore.getWebRoute('password.reset',{token: 'foo'}).path" method="get" @submit.prevent="getRepo">
          <div class="mb-5 mt-5">
            <label for="repoName" class="block mb-2 text-gray-900 dark:text-white text-xl font-bold">Repository Name</label>
            <span>At the end of repository name will be appended a random string to identify between others.</span>
            <input type="text" id="repoName" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required placeholder="name your repo..">
            <br/>
          ...
          <button type="submit" @click="getRepo" class="shadow-lg text-white font-semibold rounded-full px-5 py-1 bg-violet-500 hover:bg-violet-600 active:bg-violet-700 focus:outline-none focus:ring focus:ring-violet-300 ">Send</button>        
        </form>
      </div>
    </div>
  </div>
  <!-- <main class="w-full">
    <section class="m-3 rounded-xl border-slate-300 bg-slate-50 text-black text-left py-3 shadow-xl">
    </section>
  </main> -->
</template>