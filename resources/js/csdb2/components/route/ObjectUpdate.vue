...
<template>
  ...
  <div class="w-full text-center mb-3">
    <button :class="[showEditor ? 'border-b-black border-b-4' : '' ,'button-nav']" @click="showEditor = !showEditor">Editor</button>
    <button :class="[showUpload ? 'border-b-black border-b-4' : '' ,'button-nav']" @click="showUpload = !showUpload">Upload</button>
  </div>

  <div v-show="showUpload" class="mb-3">
    <!-- file upload -->
    <form :action="techpubStore.getWebRoute('api.post_update_csdb_object').path" method="POST" @submit.prevent="submit($event)" enctype="multipart/form-data">
      ...
      <div class="mb-5">
        ...
        <input type="file" id="description" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required placeholder="describe your project.."/>
      </div>          
  ...
  <div v-show="showEditor" class="h-max mt-3">
    <h2>XML Editor</h2>
    <div id="xml-editor-container" class="text-xl"></div>
    <br/>
    <div class="flex justify-end">
      <form method="POST" :action="techpubStore.getWebRoute('api.post_update_csdb_object').path" @submit.prevent="submit($event)">
        <button type="submit" name="button" value="commit" class="button bg-violet-400 text-white hover:bg-violet-600">Commit</button>
        <button type="submit" name="button" value="update" class="button bg-violet-400 text-white hover:bg-violet-600">Update</button>
      </form>
    </div>
  </div>
</template>