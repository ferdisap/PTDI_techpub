  ...
  <div class="comment mt-2">
    ...
    <div class="comment-list">
      ...
      <form :id="comments.cbHomeId" @submit.prevent="submit">
        <component :is="listComment" />
        <!-- <button class="material-icons send" type="submit">send</button> -->
      </form>
    </div>

    <!-- <div class="flex content-center font-semibold text-sm italic">Insert comment:
      <span @click="comments.editor.isShow = !comments.editor.isShow" class="material-symbols-outlined cursor-pointer">{{
        comments.editor.isShow ?
        'arrow_drop_down' : 'arrow_right' }}</span>
    </div> -->
    <!-- <form @submit.prevent="submit"> -->
      <!-- parent comment
      <input class="text-sm hidden" name="parentCommentFilename" value="COM-CN235-0001Z-Q-2024-67800_EN-US.xml" />
      <input class="text-sm hidden" name="previousCommentFilename" value="" />
      comment text editor
      <div class="flex items-end">
        <text-editor v-pre name="commentContentSimplePara" id="commentContentSimplePara" class="w-full" />
        <div class="flex ml-2">
          <span @click="preferences" class="material-symbols-outlined cursor-pointer text-sm">tune</span>
          <button type="submit"
        ...
        <div class="w-full text-center mb-2 relative">
          ...
          <div class="flex items-center mt-1 text-left mb-2">
            <div class="w-1/2 mr-1">
              ...
              <input modal-input-name="languageIsoCode" name="languageIsoCode" id="languageIsoCode" class="mr-2 p-2 rounded-md w-full" value="en" />
            </div>
            <div class="w-1/2 ml-1">
              <label for="countryIsoCode" class="text-sm mr-2 font-semibold italic">Country:</label>
              <input modal-input-name="countryIsoCode" name="countryIsoCode" id="countryIsoCode" class="p-2 rounded-md w-full" value="US" />
            </div>
          </div>
          ...
          <div class="relative text-left mb-2">
            ...
            <input placeholder="DMC..." type="text" class="p-2 w-full ml-1 inline text-sm rounded-md border"
              ...
              modal-input-name="brexDmRef" name="brexDmRef" dd-input="filename,path" dd-type="csdbs"
              dd-route="api.get_object_csdbs" dd-target="self">
          </div>
          ...
          <div class="flex items-center mb-2">
            ...
            <input modal-input-name="securityClassification" name="securityClassification" id="securityClassification" placeholder="eg:. 05" value="01"
          ...
          <div class="flex items-center mb-2">
            ...
            <select id="commentPriorityCode" modal-input-name="commentPriorityCode" name="commentPriorityCode" class="p-2 rounded-md">
          ...
          <div class="relative text-left mb-2">
            ...
            <input class="block p-2 w-full ml-1 rounded-md" :value="$route.params.filename" dd-input="filename,path"
              dd-type="csdbs" dd-route="api.get_object_csdbs" dd-target="self-append" modal-input-name="commentRefs" name="commentRefs" />
          </div>
    ...
    <ContextMenu :id="comments.cmId">
      <div @click.stop.prevent="comments.CB.reply(comments.modalId)" class="flex hover:bg-gray-100 py-1 px-2 rounded cursor-pointer text-gray-900">
      ...
      <div v-if="comments.CB.containerEditorId" @click.stop.prevent="emitter.emit('Modal-show', {modalId: comments.modalId})" class="flex hover:bg-gray-100 py-1 px-2 rounded cursor-pointer text-gray-900">
        <div class="text-sm">preferences</div>
      </div>
      <div v-if="comments.CB.containerEditorId" @click.stop.prevent="comments.CB.cancel" class="flex hover:bg-gray-100 py-1 px-2 rounded cursor-pointer text-gray-900">
