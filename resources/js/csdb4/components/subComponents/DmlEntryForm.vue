...
export default {
  ...
  methods: {
    searchDialog(event, state) {
      if (state) {
        let callback = (DropdownEvt) => {
          if (DropdownEvt) {
            if (['isEnterKeyPress', 'isLeftClick'].includes(DropdownEvt[0])) {
              event.target.innerHTML = DropdownEvt[1];
              this.$parent.turnOnSearchDialog(null, false);
            }
            else if (DropdownEvt[0] === 'isEscapeKeyPress') {
              this.$parent.turnOnSearchDialog(null, false);
            }
          }
        };
        this.$parent.turnOnSearchDialog(callback);
      };
    },
    /** 
  ...
  mounted() {
    console.log(this.$props.dmlContent);
    ...
    this.emitter.on('dmlEntry_sorted', this.dmlEntry_sorted); // trigger ada di DetailDML.vue
    this.emitter.on('add_dmlEntry_commentRef', this.add_dmlEntry_commentRef);
  },
}
</script>
<style>
