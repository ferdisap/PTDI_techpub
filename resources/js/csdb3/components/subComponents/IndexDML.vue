...
<template>
  <div class="IndexDML" v-if="responsedata_get_dml_list">
    ...
    <div class="flex justify-center items-center">
      <button @click="goto(responsedata_get_dml_list.current_page > 0 ? responsedata_get_dml_list.current_page - 1 : 1)"
      ...
      <button
        @click="goto(responsedata_get_dml_list.current_page < responsedata_get_dml_list.last_page ? responsedata_get_dml_list.current_page + 1 : responsedata_get_dml_list.last_page)"
  ...
  <div class="IndexCSL" v-if="responsedata_get_csl_list">
      ...
      <table class="w-full table-cell">
        ...
        <tbody>
          <tr v-for="dml in responsedata_get_csl_list.data">
            ...
            <td>{{ dml.initiator.name == techpubStore.Auth.name ? 'self' : dml.initiator.name }} </td>
            <td :class="[dml.remarks.stage == 'staged' ? 'bg-green-500' : 'bg-yellow-500']">{{ dml.remarks.stage }}</td>
            <td><a class="material-icons text-blue-600 has-tooltip-arrow" data-tooltip="Detail"
                :href="techpubStore.getWebRoute('', { filename: dml.filename }, Object.assign({}, $router.getRoutes().find((route) => route.name == 'DetailDML'))).path">details</a>
            </td>
          </tr>
        </tbody>
    ...
    <div class="flex justify-center items-center">
      <button @click="goto(responsedata_get_csl_list.current_page > 0 ? responsedata_get_csl_list.current_page - 1 : 1)"
        class="material-symbols-outlined">navigate_before</button>
      <form @submit.prevent="goto" class="inline-block">
        <input v-model="page" class="w-6" />
      ...
      <button
      @click="goto(responsedata_get_csl_list.current_page < responsedata_get_csl_list.last_page ? responsedata_get_csl_list.current_page+1 : responsedata_get_csl_list.last_page )"
      class="material-symbols-outlined">navigate_next</button>
  </div>
</div>
...
<!-- </div> -->
<AnalyzeDML v-if="filenameAnalysis" :filename="filenameAnalysis" /></template>