  ...
  <xsl:template name="transformICNMetaFile">
    ...
    <div class="icnMetadataFile hidden">
      ...
      <xsl:for-each select="//imfContent/icnVariation">
        ...
        <xsl:for-each select="icnInfoItemGroup/icnInfoItem">
          <span class="{@icnInfoItemType}"><xsl:apply-templates select="simplePara"/></span>
        </xsl:for-each>

        <map name="{$mapName}">
          <xsl:for-each select="icnContents/icnObjectGroup/icnObject">
          <!-- sementara semua children icnObject diabaikan dulu -->
          <area id="{@icnObjectIdent}" name="{@icnObjectName}" type="{@icnObjectType}" alt="{@icnObjectTitle}" title="{@icnObjectTitle}" description="objectDescr" shape="poly" coords="{@objectCoordinates}"/>
        </xsl:for-each>
        </map>

        <xsl:for-each select="icnSupportingFiles">
          <xsl:apply-templates select="icnSourceFiles/icnSupportingFilesDescr"/>
          <xsl:apply-templates select="icnSourceFiles/refs"/>
          <xsl:apply-templates select="icnResourceFiles/icnSupportingFilesDescr"/>
          <xsl:apply-templates select="icnResourceFiles/refs"/>
        </xsl:for-each>
